...
class ClassifierModel(nn.Module):
    def __init__(self, input_size, output_size, conv_features=[16]):
        ...
        for f_size in conv_features:
            # for each feature we define a conv layer
            self.convs += [nn.Conv1d(prev_f_size, f_size, 1)]
            prev_f_size = f_size
        self.fc1 = nn.Linear(prev_f_size * input_size, 10)
    ...
    for fold, (train_idx, valid_idx) in enumerate(kf.split(train_df[feat_cols], train_df[target_col])):
        ...
        for epoch in range(epochs):
            ...
            while True:
                ...
                batch_x = torch.Tensor(train_x[batch_idx])
                batch_y = torch.Tensor(train_y[batch_idx])
                current = end
